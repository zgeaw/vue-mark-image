  ...
  $.fn.ZoomMark = function(method){
    ...
    this.publicMethods = {
      init:function(options){
        ...
        mImg = mContainer.find('#containerImg');
        if(!mImg){
        ...
        addCssRule('.mark:hover','cursor:hand',0);
				...
				// 绑定默认值
        options.markList.forEach(function(item){
          mContainer.publicMethods.mark(item);
        ...
        mContainer.mousedown(function(event){
          ...
          if(event.which == 1){
            ...
            if(targetId.indexOf('mark_') >=0){
              settings.itemClick(targetId.replace('mark_', ''));							
              return;
            }
						// 禁止添加
            ...
            mContainer.publicMethods.mark({x: event.pageX- mContainer.offset().left, y: event.pageY- mContainer.offset().top});
            settings.afterMark(mMarks);
      ...
      mark:function(obj){
        ...
        var markColor = obj.color || data.settings.markColor;
        mMarks.push({
          'id': obj.id || lastMarkId+1,
          'x': obj.x,
          'y': obj.y,
          'name': obj.name || '',
          'content': obj.content || '',
        ...
        var newId = obj.id ? obj.id : newMark.id;
        mContainer.append($('<div class="vue-mark" id="mark_'+newId+'" style="background-color:'+markColor+'"></div>'));
        var object = $('#mark_'+newId);
        object.css('left', obj.x);
        object.css('top', obj.y);
      ...
      getMark: function(){
        var list = [];
        mContainer.data('ZoomMarkData').mMarks.forEach(function(item){
          if(item.available){
            list.push(item);
          }
        });
